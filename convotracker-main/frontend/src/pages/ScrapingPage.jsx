...
import { getScrapingLogs, triggerScraping, checkHealth, cleanupExpiredConvocatorias } from '../services/api';
...
export default function ScrapingPage() {
  ...
  const handleCleanupExpired = async () => {
    setRunning(true); setResult(null);
    if (!(await pingBackend())) {
      setResult({ type: 'error', message: 'Backend no disponible.' });
      setRunning(false); return;
    }
    try {
      const res = await cleanupExpiredConvocatorias();
      setResult({ type: 'success', message: `Se actualizaron ${res.updated} convocatorias vencidas.` });
      await fetchData();
    } catch (e) {
      setResult({ type: 'error', message: `Error: ${e?.response?.data?.detail || e?.message || 'error de red'}.` });
    }
    setRunning(false);
  };
    ...
    <div className="mx-auto flex flex-col gap-4 sm:gap-6" style={{ maxWidth: 860, padding: '0 12px' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          >
            {running ? <Loader2 style={{ width: 15, height: 15, animation: 'spin 1s linear infinite' }} /> : <Clock style={{ width: 15, height: 15 }} />}
            {running ? 'Procesando…' : 'Limpiar vencidas'}
          </button>
          <button
